<?php
namespace SK\Digidoc\Example\DigiDocService;

use SK\Digidoc\Example\Common\CommonUtils;
use SK\Digidoc\Example\Exception\SoapClientException;
...
final class DigiDocService
{
    ...
    private function invoke($serviceName, $params)
    {
        try {
            ...
            if (LOG_ALL_DDS_REQUESTS_RESPONSES) {
                CommonUtils::debugLog($serviceName.'Request: \''.$this->client->__getLastRequest().'\'');
                CommonUtils::debugLog($serviceName.'Response: \''.$this->client->__getLastResponse().'\'');
            }
    ...
    */
    ...
    {
        CommonUtils::debugLog('Following request caused an error: \''.$this->client->__getLastRequest().'\'');
        $detail_message = '';
        if (isset($exception->detail) && isset($exception->detail->message)) {
            $detail_message = $exception->detail->message;
        }
        $code = (!!$exception->getCode() ? $exception->getCode().' - ' : '').$exception->getMessage();

        throw new SoapClientException("There was an error invoking $service_name: ".($code.(empty($detail_message)
                    ? ''
                    : ' - '.$detail_message)));
    }
